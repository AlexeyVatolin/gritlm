...
class GritLM(torch.nn.Module):
    def __init__(
        ...
        mode: Literal["unified", "embedding", "generative"] = "unified",
        pooling_method: Literal["cls", "lasttoken", "mean", "weightedmean"] = "mean",
        normalized: bool = True,
        projection: int | None = None,
        is_inference: bool = True,
        embed_eos: str = "",
        attn: Literal["bbcc", "cccc", "bb", "cc"] | None = "bbcc",
        device: str = "cuda" if torch.cuda.is_available() else "cpu",
        **kwargs: Any,  # Passed to the model, e.g. `attn_implementation`, `torch_dtype` etc.
    ) -> None:
        super().__init__()
        if mode == "embedding":
            ...
            if hasattr(self.model, "model"):  # LLama2 & Mistral
                ...
                raise ValueError(
                    "Could not find attribute to use for embedding: ", self.model
                )
        ...
        if (self.attn is not None) and self.attn not in ["bbcc", "cccc", "bb", "cc"]:
            raise ValueError(
                f"Mixed attention no longer supported: {self.attn}. Only bbcc, cccc, bb, cc are supported"
            )

        print(
            f"Created GritLM: {self.model.dtype} dtype, {pooling_method} pool, {mode} mode, {attn} attn"
        )
        ...
        if is_inference:
            ...
            if self.embed_eos:
                assert (
                    self.embed_eos in self.tokenizer.vocab
                ), f"EOS token {self.embed_eos} not in vocab"
            self.model.eval()
            if (
                "device_map" not in kwargs
                and not (kwargs.get("load_in_4bit", False))
                and not (kwargs.get("load_in_8bit", False))
            ):
                self.model.to(self.device)
                # Parallelize embedding model unless a specific device is specified, e.g. `cuda:1`
                if (mode == "embedding") and (
                    (isinstance(self.device, str) is False) or (":" not in self.device)
                ):
                    self.num_gpus = torch.cuda.device_count()
                    if self.num_gpus > 1:
                        print(
                            f"----------Using {self.num_gpus} data-parallel GPUs----------"
                        )
                        self.model = torch.nn.DataParallel(self.model)

    def encode_queries(
        self, queries: list[str] | str, **kwargs: Any
    ) -> NDArray[np.float32]:
    ...
    def encode_corpus(
        self,
        corpus: list[str] | str | list[dict[str, str]] | dict[str, str],
        **kwargs: Any,
    ) -> NDArray[np.float32]:
    ...
    def encode(
        ...
        **kwargs: Any,
    ) -> NDArray[np.float32] | tuple[NDArray[np.float32], list[Any]]:
        if self.num_gpus > 1:
        ...
        for start_index in tqdm(
            ...
            outputs = (
                getattr(self.model, self.embedding_attr)
                if self.embedding_attr
                else self.model
            )(**inputs)
            ...
            if get_cache:
                ...
                assert (
                    len(all_kv_caches) == 0
                ), "Can only get cache for one batch at a time"
            ...
            if (
                (instruction)
                and (embed_instruction is False)
                and ("mean" in self.pooling_method)
            ):
            ...
            embeddings = self.pooling(
                last_hidden_state, inputs["attention_mask"], recast=recast
            )
            ...
            if self.normalized:
                ...
                embeddings = torch.nn.functional.normalize(embeddings, dim=-1).to(
                    in_dtype
                )
    ...
    def pooling(
        ...
        if self.pooling_method == "cls":
            ...
            embedding = torch.gather(
                hidden_state * input_mask_expanded, 1, gather_indices
            ).squeeze(dim=1)
        elif self.pooling_method in ["mean", "weightedmean"]:
            if self.pooling_method == "weightedmean":
                attention_mask *= attention_mask.cumsum(
                    dim=1
                )  # [0,1,1,1,0,0] -> [0,1,2,3,0,0]
